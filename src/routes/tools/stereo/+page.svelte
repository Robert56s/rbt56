<script lang="ts">
	...
	let lengthMode = $state<'longest' | 'shortest' | 'custom'>('longest');
	let customSec = $state(30);
	let rateMode = $state<'auto' | number>('auto');
	let fadeMs = $state(5);

	let format = $state<'wav16' | 'wav24' | 'mp3'>('wav16');
	...
	let result = $state<AudioBuffer | null>(null);
...
<article>
	...
	<section class="panel">
		...
		<div class="sides">
			<SideCard channel={left} label="Left channel" letter="L" tone="var(--left)" />
			<SideCard channel={right} label="Right channel" letter="R" tone="var(--right)" />
		</div>
	...
	<section class="panel">
		...
		<div class="result">
			<Waveform
				buffer={result}
				colors={['var(--left)', 'var(--right)']}
				height={110}
				empty={ready ? 'not rendered yet' : 'no source loaded'}
			/>
			<div class="legend">
				<span><i style="background: var(--left)"></i>top: left channel</span>
				<span><i style="background: var(--right)"></i>bottom: right channel</span>
			</div>
		</div>
