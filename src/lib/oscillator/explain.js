...
export function explainStabilizer(design) {
	...
	if (limiter.kind === 'jfet') {
		...
		blocks.push(
			...
			p(
				`The control voltage comes from a peak detector: a diode charges Cdet to the output's negative peak, a divider Ra Rb scales it to what the gate needs, and the same divider bleeds the capacitor slowly. Two details set the numbers. The capacitor droops only about 2 percent between peaks, so the diode conducts for a fifteenth of each cycle and its current then is some 30 times the average: its drop is the one at that current, about half a volt, not the 0.3 V the average would suggest. And the two averaging resistors draw a little current from the divider, which lowers its output by a few percent. Here the detector delivers ${n2(-limiter.vPeak)} V at the peak node for a ${formatVolts(amplitude)} output and the divider passes ${(100 * limiter.rb / (limiter.ra + limiter.rb)).toFixed(0)} percent of it; the loop settles where the gain is exactly ${n2(requiredGain)}, which the rounded parts put at about ${formatVolts(limiter.amplitudeActual ?? NaN)} peak. LTspice lands within about 1 percent of that.`
			),
