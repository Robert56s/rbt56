...
export const MP3_RATES = [48000, 44100, 32000, 24000, 22050, 16000, 12000, 11025, 8000];

export const MP3_BITRATES = [96, 128, 160, 192, 256, 320];

export function supportsMp3(sampleRate: number): boolean {
	return MP3_RATES.includes(sampleRate);
}

function toInt16(data: Float32Array, frames: number, from: number, into: Int16Array): void {
...
export async function encodeMp3(
	...
	if (!supportsMp3(buffer.sampleRate)) {
		throw new Error(
	...
	for (let offset = 0; offset < total; offset += block) {
		...
		const encoded =
			channels > 1 ? encoder.encodeBuffer(slabL, slabR) : encoder.encodeBuffer(slabL);
